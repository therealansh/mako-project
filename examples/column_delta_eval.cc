...
public:
    ...
    void run_payment_simulation(int num_payments) {
        ...
        double throughput = num_payments * 1000.0 / (duration.count() > 0 ? duration.count() : 1);
        double avg_latency_us = (duration.count() * 1000.0) / num_payments;
        printf("Throughput: %.2f payments/sec\n", throughput);
        printf("Avg latency: %.2f us/payment\n", avg_latency_us);
        
        // Print metrics immediately (before any potential crash)
#if MAKO_ENABLE_COLUMN_DELTAS
    ...
    void print_metrics() {
        // Metrics already printed in run_payment_simulation
    }
...
void run_evaluation(abstract_db *db, bool enable_column_delta) {
    printf("\n=== Running with Column-Delta %s ===\n", enable_column_delta ? "ENABLED" : "DISABLED");
    
#if MAKO_ENABLE_COLUMN_DELTAS
    mako::setColumnDeltasEnabled(enable_column_delta);
    printf("Runtime flag set to: %s\n", mako::isColumnDeltasEnabled() ? "ON" : "OFF");
#else
    (void)enable_column_delta;
    printf("Column-delta compiled out (MAKO_ENABLE_COLUMN_DELTAS=0)\n");
#endif
    ...
    // Note: skip delete to avoid cleanup crash (known codebase issue)
}

int main(int argc, char *argv[]) {
    // Parse command line: --baseline (OFF) or --column-delta (ON, default)
    bool enable_column_delta = true;
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--baseline") == 0) {
            enable_column_delta = false;
        } else if (strcmp(argv[i], "--column-delta") == 0) {
            enable_column_delta = true;
        }
    }
    ...
    run_evaluation(db, enable_column_delta);
    
    // Note: skip delete db to avoid cleanup crash
