...
# Script to test 1-shard YCSB experiments with replication
# Each shard should:
...
# Start shard 0 in background with YCSB benchmark
echo "Starting shard 0 with YCSB benchmark..."
nohup bash bash/shard.sh 1 0 $trd localhost 0 1 ycsb > $script_name\_shard0-localhost-$trd.log 2>&1 &
nohup bash bash/shard.sh 1 0 $trd learner 0 1 ycsb > $script_name\_shard0-learner-$trd.log 2>&1 &
nohup bash bash/shard.sh 1 0 $trd p2 0 1 ycsb > $script_name\_shard0-p2-$trd.log 2>&1 &
sleep 1
nohup bash bash/shard.sh 1 0 $trd p1 0 1 ycsb > $script_name\_shard0-p1-$trd.log 2>&1 &
SHARD0_PID=$!
sleep 2
...
# Check each shard's output
{
    ...
    # Check for agg_throughput keyword (YCSB benchmark completed)
    if grep -q "agg_throughput" "$log"; then
        echo "  ✓ Found 'agg_throughput' keyword (YCSB completed)"
        grep "agg_throughput" "$log" | tail -1 | sed 's/^/    /'
    else
        echo "  ✗ 'agg_throughput' keyword not found (YCSB may not have completed)"
        failed=1
    fi
}
