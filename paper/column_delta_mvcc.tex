...
We implement Column-Delta MVCC in Mako, a high-performance distributed transactional key-value store. Evaluation on TPC-C shows 69.2\% storage savings with less than 1\% throughput overhead at scale. Our implementation requires only 481 lines of generic infrastructure code and demonstrates that significant storage optimizations can be achieved through careful exploitation of schema information in modern database systems.
\end{abstract}
...
\midrule
1 & 91.1 & 65.6 & 92.6 & 0.0104 & 0.0103 \\
2 & 179.4 & 165.3 & 180.6 & 0.0106 & 0.0105 \\
4 & 347.2 & 329.9 & 347.1 & 0.0110 & 0.0110 \\
6 & 516.1 & 402.9 & 508.3 & 0.0111 & 0.0112 \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{No Column Split} shows the cost of removing manual vertical partitioning of the STOCK table (merging \texttt{stock} and \texttt{stock\_data} into a single table), which is orthogonal to column-delta (instrumented on Customer, Warehouse, and District tables in Payment). Throughput drops by 5--28\% depending on thread count due to larger row sizes on the NewOrder critical path.
\end{itemize}
...
\addplot[color=orange,mark=diamond*,thick] coordinates {
    (1, 65.6) (2, 165.3) (4, 329.9) (6, 402.9)
};
