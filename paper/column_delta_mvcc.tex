...
\begin{abstract}
Multi-Version Concurrency Control (MVCC) is fundamental to modern distributed databases, but traditional implementations store full-row copies for every update, causing severe storage amplification for wide tables with highly localized updates. We present Column-Delta MVCC, a schema-aware MVCC variant that encodes only modified columns in version chains by leveraging compile-time schema metadata, enabling a generic delta layer without per-table specialization---implemented in just 481 lines of infrastructure code in Mako, a high-performance distributed transactional key-value store. On TPC-C, Column-Delta MVCC achieves 69.2\% storage savings with less than 1\% throughput overhead compared to baseline, while removing manual vertical partitioning (merging split tables into single tables) degrades throughput by 8--11\%, demonstrating that Column-Delta delivers substantial storage benefits without the performance penalty of naive schema modifications.
\end{abstract}
