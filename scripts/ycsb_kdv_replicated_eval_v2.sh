#!/bin/bash
#
# YCSB KDV vs Baseline Evaluation in Replicated Mode (v2)
#
# This script runs the internal YCSB benchmark on a 1‑shard Paxos‑replicated
# configuration and collects, for both baseline and KDV:
...
#
# It sweeps:
#   - Several YCSB‑style workload mixes (A/B/F‑like)
#   - Multiple record sizes
#   - Multiple update sizes (small / half / large)
#
# Usage:
#   THREADS=6 RUNTIME=60 NUM_KEYS=100000 \
#     ./scripts/ycsb_kdv_replicated_eval_v2.sh [output_csv]
#
# Defaults:
#   THREADS   = 6 worker threads per replica
#   RUNTIME   = 60 seconds per experiment
#   NUM_KEYS  = 100000 YCSB keys
#
...
OUTPUT_CSV="${1:-results/ycsb_kdv_replicated_eval_v2.csv}"
RESULTS_DIR="${PROJECT_ROOT}/results/ycsb_kdv_replicated_eval_v2"
mkdir -p "${RESULTS_DIR}"
...
# Disable advancer for replicated YCSB runs. Advancer is only required
# for legacy TPCC experiments and is known to cause instability when
# combined with YCSB + RocksDB KDV evaluation.
export MAKO_DISABLE_ADVANCER=1

# Workload definitions: label:read,write,rmw,scan:description
# Map roughly onto standard YCSB workloads:
#   - A: balanced read/update
#   - B: read‑heavy
#   - F: update‑heavy
WORKLOADS=(
  "A:50,0,50,0:YCSB‑A‑like (balanced read/update)"
  "B:95,0,5,0:YCSB‑B‑like (read‑heavy)"
  "F:20,0,80,0:YCSB‑F‑like (update‑heavy)"
)
...
run_one_experiment() {
  local workload_label="$1"   # e.g., A
  local workload_mix="$2"     # e.g., 50,0,50,0
  ...
  log_err "============================================================"
  log_err "Experiment: workload=${workload_label}  mode=${mode}"
  ...
  local run_dir="${RESULTS_DIR}/${workload_label}_r${record_size}_u${update_bytes}_${mode}"
  mkdir -p "$run_dir"
  ...
  echo "${workload_label},${mode},${read_pct},${write_pct},${rmw_pct},${scan_pct},${record_size},${update_bytes},${NUM_KEYS},${THREADS},${RUNTIME},${kdv_enabled},${metrics}"
}

main() {
  cd "$PROJECT_ROOT"

  log_err "YCSB KDV vs Baseline Replicated Evaluation (v2)"
  ...
  # CSV header
  local header="workload_label,mode,read_pct,write_pct,rmw_pct,scan_pct,record_size,update_bytes,num_keys,threads,runtime_sec,kdv_enabled,throughput,avg_latency_ms,paxos_bytes,original_bytes,encoded_bytes,compression_ratio_pct,rocksdb_bytes"
  ...
  for workload_spec in "${WORKLOADS[@]}"; do
    ...
    log_err "------------------------------------------------------------"
    log_err "Workload ${label} (${desc}): mix=${mix}"
    log_err "------------------------------------------------------------"
  ...
  log_err "  - throughput difference:      mode=kdv vs baseline"
  log_err "  - latency overhead:           avg_latency_ms (kdv vs baseline)"
  log_err "  - storage reduction:          rocksdb_bytes (kdv vs baseline)"
  log_err "  - network bandwidth savings:  paxos_bytes (kdv vs baseline)"
  log_err "============================================================"
}
